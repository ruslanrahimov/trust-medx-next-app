...
export default function TreatmentAbroadHero({ dict, lang }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 py-20 text-center">
        ...
        <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-tight">
          <div className="hero-title-line mb-2">
            <span className="text-[#4A3B2C] font-[family-name:var(--font-fraunces)]">
              {dict.treatmentAbroad.hero.title}
            </span>
          </div>
          <div className="hero-title-line relative inline-block">
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#2C5F5D] via-[#5FA8A3] to-[#2C5F5D] font-[family-name:var(--font-fraunces)]">
              {dict.treatmentAbroad.hero.titleAccent}
            </span>
        ...
        {/* Subtitle */}
        <p className="hero-subtitle text-lg md:text-xl text-[#4A3B2C]/80 mb-4 max-w-4xl mx-auto font-light leading-relaxed font-[family-name:var(--font-dm-sans)]">
          {dict.treatmentAbroad.hero.subtitle}
        </p>

        {/* Description */}
        <p className="hero-description text-base md:text-lg text-[#4A3B2C]/60 max-w-3xl mx-auto mb-12 leading-relaxed font-[family-name:var(--font-dm-sans)]">
          {dict.treatmentAbroad.hero.description}
        </p>

        {/* Scroll Indicator */}
        <div className="scroll-indicator flex flex-col items-center gap-2 text-[#5FA8A3]">
          <span className="text-xs uppercase tracking-widest font-medium font-[family-name:var(--font-dm-sans)]">
            {lang === 'ru' && 'Выберите страну'}
            {lang === 'en' && 'Choose Country'}
            {lang === 'ar' && 'اختر البلد'}
          </span>
          <svg
            width="24"
