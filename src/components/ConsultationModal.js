...
export default function ConsultationModal({ isOpen, onClose, dict, lang }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Modal Card */}
        <div className="relative bg-[#FEFBF6]/98 backdrop-blur-xl rounded-[2rem] shadow-2xl border border-[#D4A574]/20 overflow-hidden">
          {/* Decorative gradient overlay */}
          <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[#5FA8A3] via-[#D4A574] to-[#5FA8A3]" />
          ...
          <button
            onClick={handleClose}
            className="absolute top-4 right-4 z-10 w-9 h-9 flex items-center justify-center rounded-full bg-white/80 backdrop-blur-sm border border-[#4A3B2C]/10 text-[#4A3B2C] hover:bg-[#D4A574]/20 hover:border-[#D4A574] hover:text-[#D4A574] transition-all duration-300 hover:rotate-90 hover:scale-110 shadow-lg"
            aria-label="Close"
          >
            <X className="w-4 h-4" />
          </button>
          ...
          <div className="relative p-5 md:p-6">
            {/* Header */}
            <div className="text-center mb-4 space-y-1.5" style={{ animation: 'slideDown 0.6s ease-out 0.1s both' }}>
              {/* Badge */}
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/60 backdrop-blur-sm border border-[#D4A574]/30">
                <div className="w-2 h-2 rounded-full bg-[#D4A574] animate-pulse" />
                <span className="text-xs font-semibold text-[#4A3B2C]/80 uppercase tracking-wider" style={{ fontFamily: 'var(--font-dm-sans)' }}>
              ...
              {/* Title */}
              <h2
                className="text-2xl md:text-3xl font-bold text-[#4A3B2C] leading-tight"
              ...
              {/* Subtitle */}
              <p className="text-sm text-[#4A3B2C]/70 max-w-xl mx-auto" style={{ fontFamily: 'var(--font-dm-sans)' }}>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              {/* Name Fields - Grid */}
              <div className="grid md:grid-cols-2 gap-3" style={{ animation: 'slideUp 0.6s ease-out 0.2s both' }}>
                {/* First Name */}
                <div className="group/field">
                  <label
                    htmlFor="firstName"
                    className="block text-sm font-semibold text-[#4A3B2C] mb-1.5"
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-lg border-2 border-[#4A3B2C]/10 bg-white/60 text-[#4A3B2C] placeholder-[#4A3B2C]/40 transition-all duration-300 focus:outline-none focus:border-[#D4A574] focus:bg-white focus:shadow-lg focus:shadow-[#D4A574]/20 hover:border-[#4A3B2C]/20 text-base"
                    style={{ fontFamily: 'var(--font-dm-sans)' }}
                    dir={lang === 'ar' ? 'rtl' : 'ltr'}
                  />
                </div>
              ...
              {/* Contact Fields - Grid */}
              <div className="grid md:grid-cols-2 gap-3" style={{ animation: 'slideUp 0.6s ease-out 0.3s both' }}>
                {/* Email */}
                <div className="group/field">
                  <label
                    htmlFor="email"
                ...
                <div className="group/field">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    required
                    placeholder={dict?.consultationModal?.placeholders?.phone || '+7 (___) ___-__-__'}
                    className="w-full px-3 py-2 rounded-lg border-2 border-[#4A3B2C]/10 bg-white/60 text-[#4A3B2C] placeholder-[#4A3B2C]/40 transition-all duration-300 focus:outline-none focus:border-[#D4A574] focus:bg-white focus:shadow-lg focus:shadow-[#D4A574]/20 hover:border-[#4A3B2C]/20 text-base"
                    style={{ fontFamily: 'var(--font-dm-sans)' }}
                    dir="ltr"
                  />
              ...
              <div style={{ animation: 'slideUp 0.6s ease-out 0.4s both' }}>
                <label
                  htmlFor="specialty"
                  className="block text-sm font-semibold text-[#4A3B2C] mb-2"
                  style={{ fontFamily: 'var(--font-dm-sans)' }}
                >
                  ...
                  {specialties.map((spec) => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative p-2.5 rounded-lg border-2 transition-all duration-300 text-left group/spec ${
                          isSelected
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          {isSelected && (
                            <div className="w-1.5 h-1.5 rounded-full bg-[#D4A574] animate-pulse flex-shrink-0" />
                          )}
                        </div>
                      </button>
                    );
              ...
              <div style={{ animation: 'slideUp 0.6s ease-out 0.5s both' }}>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 rounded-lg border-2 border-[#4A3B2C]/10 bg-white/60 text-[#4A3B2C] placeholder-[#4A3B2C]/40 transition-all duration-300 focus:outline-none focus:border-[#D4A574] focus:bg-white focus:shadow-lg focus:shadow-[#D4A574]/20 resize-none hover:border-[#4A3B2C]/20 text-base"
                  style={{ fontFamily: 'var(--font-dm-sans)' }}
                  dir={lang === 'ar' ? 'rtl' : 'ltr'}
                />
              </div>
              ...
              <div style={{ animation: 'slideUp 0.6s ease-out 0.6s both' }}>
                <button
                  ...
                  className="group/btn w-full relative overflow-hidden px-5 py-2.5 rounded-lg bg-gradient-to-br from-[#5FA8A3] to-[#4A9691] text-white font-bold text-base shadow-xl shadow-[#5FA8A3]/30 transition-all duration-500 hover:shadow-2xl hover:shadow-[#5FA8A3]/40 hover:-translate-y-0.5 hover:scale-[1.01] disabled:opacity-70 disabled:cursor-not-allowed disabled:hover:translate-y-0 disabled:hover:scale-100"
                  style={{ fontFamily: 'var(--font-dm-sans)' }}
                >
                  {/* Shimmer effect */}
            ...
            {/* Privacy Notice */}
            <p
              className="text-xs text-[#4A3B2C]/50 text-center mt-2"
